...
/**
 * 是否可用「访达选文件夹」：仅在本机 macOS 上运行 Portal 时有效（服务端执行 osascript）。
 * 浏览器无法把访达路径交给网页，故必须由 Node 进程调起系统对话框。
 */
...
export async function POST() {
    ...
    return Response.json(
      {
        ...
        error:
          process.platform !== "darwin"
            ? "当前 Portal 未在 macOS 本机运行，无法使用访达选目录，请手动填写路径"
            : "已禁用访达选目录（STUDY_DISABLE_FOLDER_PICKER）",
      },
      { status: 501 }
    );
  }
  ...
  try {
    const script = 'POSIX path of (choose folder with prompt "请选择知识库文件夹")';
    ...
    const v = resolveAllowedFolderPath(raw);
    if (!v.ok) {
    ...
    return Response.json({ ok: false, error: combined.trim() || "选目录失败" }, { status: 500 });
  }
}
